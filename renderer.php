    ...
    public function render_gallery_view_gallery(gallery_view_gallery $widget) {
        ...
        foreach($widget->images as $image) {
            ...
            if($widget->edit) {
                ...
                $o .= $this->output->action_link(new moodle_url('/mod/gallery/view.php', $urlparams), get_string('delete','gallery'),array('onclick'=>"return confirm('".get_string('confirmdelete','gallery')."')"));
                $o .= $this->output->box_end();
                $o .= '</div>';
            } else
