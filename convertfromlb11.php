...
$lbgmids = array(123190,123203,123208,123212,123213,123214,123215,123216,123217,123218,123219,123220,123221,
	121475,121552,121558,121563,121689,121692,121695,121699,121702,121705,121708,
	115815,118252,118262,118286,119241,119248,119252,119971,121688,121818,121823,121830,122079);
echo 'Found Lightbox galleries: '.count($lbgalleries),'<br /><br />';
foreach($lbgmids as $lbgmid) {
	$lbgcm = $DB->get_record('course_modules',array('id'=>$lbgmid),'*',MUST_EXIST);	
    $course = $DB->get_record('course', array('id'=>$lbgcm->course), '*', MUST_EXIST);
	$lbgallery = $DB->get_record('lightboxgallery',array('id'=>$lbgcm->instance),'*',MUST_EXIST);
